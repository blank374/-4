#本文本是SampleForLatex的说明文本
#此模板文本提供由数学建模协会提供，林政扬编辑
此次模板提升了可读性，各位只需要该对应的题目就行。模板把必要能修改的东西已经说明，各位可以自行修改。
#注意基本的设置已经确定好了，并封装在shumo.sty宏包上，如果有问题各位可以打开宏包修改问题项（有些人可能宏包下载不齐全）
==============================================
关键词的修改：---》在\quad 后面“双十一购物节，基于目标规划的判别模型，演化博弈模型”对应修改自己的
\textbf{关键词}:\quad 双十一购物节，基于目标规划的判别模型，演化博弈模型
=============================================
代码的修改：---》“language= 自己的代码语言”，%这里开始粘贴自己的代码后面开始粘上自己的代码
\begin{center}
	\sihao \heiti 附录
	
	
	\fontsize{10pt}{16pt}		\selectfont	
	\begin{lstlisting}[ language=MATLAB,numbers=left,numberstyle=\tiny,keywordstyle=\color{blue!70},commentstyle=\color{red!50!green!50!blue!50},frame=shadowbox, rulesepcolor=\color{red!20!green!20!blue!20},escapeinside=``,xleftmargin=2em,xrightmargin=2em,aboveskip=1em] 

%这里开始粘贴自己的代码
	%%%问题一
	%%图形
	%从1到5商品分别代表着：帮宝适绿帮纸尿裤、黛珂牛油果紫苏水套装
	%WIS水润面膜、飞利浦电动牙刷HX6616、HFP熊果苷补水面膜
%
%
%
此外还提供了两个宏包，大家可以自己调用，这里不说明如何使用，对于有需求的同学可以自己进行查询。“code”的宏包是自己定义的，网上找不到，要怎么用自己的查看包里面的语句。正文其他部分请给位同学进行网上查找。

预祝大家取得好成绩。



