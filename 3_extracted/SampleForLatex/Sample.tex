...
对每个查询合并问题三二维、三维、五维的已认证路径，按五维成本去重并执行五维精确去支配，得到原始候选集 $A$。对第 $j$ 项成本定义
\begin{equation}
...
封路前后固定使用同一组 $L_j,R_j$ 与权重，不对封路后的成本重新归一化，也不把超出原候选范围的值截断。这样两种路网状态下的评分具有可比性。

\subsection{精确整数标量化}

令表中百分比整数权重为 $w_j$，$D$ 为各 $R_j$ 的最小公倍数，定义
\begin{equation}
...
h_0(v)\le h_D(v),
\end{equation}
且 $h_0$ 在所有保留边上仍一致，可作为中断图 A* 的下界\cite{astar}。A* 在完整剩余图上搜索，而非只在旧候选中选择；当 OPEN 最小下界不小于已有上界或目标出队时得到精确标量最优解。若无上界且 OPEN 耗尽，才判定不可达。
...
原候选加权误差实测最大值为 3.366\%，显著低于五维 20\% 理论界；时间最短方案在全部查询上与原图时间最优值一致。共有 80 个方案实例出现 $W_D^*<W_A$，但全部满足 $W_D^*\ge W_0^*$，验证了“评分下降来自候选近似误差消除，而非封路改善路网”的解释。

\subsection{权重敏感性与计算效率}

分别对“时间权重占比”和“起伏/复杂度合计占比”取 $\theta=0,0.1,\ldots,1$。时间权重扫描使全部 90 组查询发生过推荐切换；起伏/复杂度扫描使 80 组发生切换。NY、BAY、COL 在 11 个时间权重设置下的不同推荐数中位数分别为 5、4、5，说明推荐随权重呈分段不变并在阈值处切换。
...
从决策层面看，时间优先方案可以较小时间代价换取一定的起伏与复杂度改善，而平稳优先的时间代价在不同城市差异显著；BAY 对指定封路的最短时间损失中位数最大。结果表明，多目标候选、偏好解释和扰动后全图搜索缺一不可：只给单目标路线无法表达权衡，只筛旧候选可能误判不可达，只比较封路前后推荐分数又可能把候选误差误认为路网改善。
